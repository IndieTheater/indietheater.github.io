<script lang="ts">
	...
	import { House, Tv, User, Newspaper, Info } from 'lucide-svelte';
	...
	<div class="navbar-container">
		...
		<div class="navbar-links-desktop">
			{#each navItems as item}
				...
				>
					{#if item.icon}
						<svelte:component this={item.icon} class="mr-1 inline h-5 w-5" />
					{/if}
					{item.label}
				</a>
			{/each}
		...
		<div class="navbar-controls">
			<!-- Theme Toggle -->
			<button class="theme-toggle-enhanced" aria-label="Toggle theme" on:click={toggleTheme}>
				{#if isDark}
	...
	{#if mobileMenuOpen}
		<div class="mobile-menu">
			{#each navItems as item}
				<a
					href={item.href}
					class="mobile-nav-link {$page.url.pathname === item.href ||
					($page.url.pathname.startsWith(item.href + '/') && item.href !== '/')
						? 'active'
						: ''}"
					aria-current={$page.url.pathname === item.href ? 'page' : undefined}
